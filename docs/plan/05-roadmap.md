...
**In:** unit entities from `units.json` (scout, warrior, settler); MoveUnit;
...
**In:** FoundCity; tile yields; food/growth; production queues; **quarters
...
verifiable in browser and as a golden log; a city left on a policy stance
develops sensibly for 30 turns with zero interventions (scripted sim assert).
...
autosave; `sim` runs full AI-vs-AI games with telemetry.
...
with a winner before turn 300, twice, hash-identical; combat golden log
passes. **Playthrough gate (doc 06 §4) activates here and never turns off.**
...
first time; tech-gated units/buildings/quarters; worker improvements
(farm/mine/road) as policy-driven work with intervention override; tech tree
UI; save/load UI (IndexedDB + file export); first real saveVersion migration.
...
and a negotiated peace; AI-vs-AI wars in the harness end in peace deals (not
eternal stalemates) in ≥80% of wars; battle resolution covered by table
...
enough to bet the game on); **Science victory** (space project chain) and
**living victory-track UI** for Domination + Science.
**AC:** win a Science game vs AI in-browser; AI wins by science in ≥15% of
nightly games (it genuinely pursues it); a deliberately zero-intervention
"policies only" human playthrough remains competitive on default difficulty
(the automation-competence gate, doc 06 §7); time-per-turn telemetry flat
from turn 50 → 250.
...
**AC:** AI accepts a fair deal and rejects a lopsided one (unit-tested against
the evaluator); every opinion number on the diplomacy screen expands to its
...
agents, 4 mission types with previewed odds); naval commanders.
**AC:** found and spread a religion into a rival city in-browser; a raided
trade route visibly stings (golden log); city-state levy turns a harness war;
...
**In:** the 6-era structure finalized; **5 Turning Points** authored
...
(doc 02 §11); roster to **12 civs / 14 leaders**; balance freeze begins —
nightly bands (doc 06 §3) become merge-blocking.
...
between 1st and 4th place (measured); full-game human playthrough gate on
every victory type.
...
**generated from content JSON** (free win of data-driven design); full
